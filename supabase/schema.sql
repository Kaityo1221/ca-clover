...
-- Activity Map: one aggregate row per visible Community.
...
-- Community access claims from a CA-owned Meetup URL/ID.
...
create unique index community_access_requests_pending_key
  on public.community_access_requests(user_id,community_id)
  where status='pending';

create index community_access_requests_status_idx
  on public.community_access_requests(status,requested_at desc);
create index community_access_requests_user_idx
