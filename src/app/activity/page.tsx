    ...
    <section className="clover-card mt-5 p-5">
      ...
      <div className="mt-6 grid grid-cols-6 gap-3 md:grid-cols-12">
        {monthMap.map(([month,value])=><div key={month} className="flex min-w-0 flex-col items-center justify-end gap-2">
          <div className="flex h-36 w-full items-end justify-center rounded-xl bg-lime-50 p-1"><div className="w-full rounded-lg bg-lime-300" style={{height:Math.max(4,Math.round((Number(value.checkin_count)||0)/maxMonthly*100))+"%"}} title={"Check-in "+value.checkin_count}/></div>
