...
function patchModal(item){
 ...
 if(stamp3d){stamp3d.classList.remove("show");if(stamp3d.childNodes.length)stamp3d.innerHTML="";}
 ...
 if(img){
  ...
  img.style.opacity="1";
  if(item.avatar_url&&img.src!==item.avatar_url)img.src=item.avatar_url;
 }
 if(controls){
  const d=item.acquired_at?new Date(item.acquired_at):new Date();
  const date=Number.isNaN(d.getTime())?"":d.toLocaleDateString("ja-JP");
  const html='<div class="ca-vt-modal-badge">🧪 TEST取得済み</div><div class="ca-vt-modal-note">仮想交換で取得した表示です。正式な取得履歴には保存されていません。'+(date?'<br>テスト取得日: '+date:'')+'</div>';
...
function installRally(){
 ...
 document.addEventListener("click",ev=>{
  ...
  const back=document.getElementById("stampModalBack");
  if(close||(back&&ev.target===back))activeVirtual=null;
 },true);
 schedule();
